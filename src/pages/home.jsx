...
const Home = () => {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-slate-200 font-sans selection:bg-indigo-500/30 selection:text-white overflow-x-hidden">
        ...
        <div className="w-full max-w-4xl grid grid-cols-1 gap-6 mb-20">
          ...
          <SpotlightCard className="p-8 md:p-12 rounded-xl backdrop-blur-md">
            ...
            <p className="text-lg md:text-xl text-slate-300 font-light leading-relaxed">
              You move through a world dense with signals. Life’s true signal gets drowned out by noise—notifications impersonating social worth, lightning deals timed to attention cycles, and bottomless digital reels.
            </p>
          </SpotlightCard>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <SpotlightCard className="p-8 md:p-10 rounded-xl backdrop-blur-md">
              ...
              <p className="text-base text-slate-400 font-light leading-relaxed">
                Your neurochemistry isn’t hacked; it’s exploited. Dopamine fires on demand. Novelty arrives instantly. Motivation dulls. Focus fractures.
              </p>
            </SpotlightCard>
          ...
          <div className="order-2 lg:order-1 flex justify-center relative">
            <div className="relative w-full max-w-md aspect-square bg-white/5 border border-white/5 rounded-sm p-8 backdrop-blur-sm">
               <svg viewBox="0 0 200 200" fill="none" className="w-full h-full opacity-90">
                  ...
                  {/* Nodes */}
                  <motion.circle cx="100" cy="100" r="3" fill="white" initial={{ scale: 0 }} whileInView={{ scale: 1 }} transition={{ delay: 1 }} />
               </svg>
      ...
      <section className="w-full py-20 relative z-10 px-4">
        <div className="max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  { icon: MousePointer2, text: "Conventional tools try to push your behavior. Ours exists only to reveal it." },
                  { icon: Layers, text: "A good tool detects the pattern, locates the friction, and offers a precise lever to lift the weight." },
                  { icon: Zap, text: "It does not demand discipline. It restores gumption." },
                  { icon: Terminal, text: "This is the first step toward the craft of living.", highlight: true }
                ].map((item, i) => (
                  <SpotlightCard key={i} className={`p-8 min-h-[160px] flex flex-col justify-between rounded-lg ${item.highlight ? 'bg-white/10' : ''}`}>
                     <div className="text-indigo-400 mb-2">
                        <item.icon size={20} strokeWidth={1.5} />
                     </div>
                     <p className={`text-sm leading-relaxed ${item.highlight ? 'text-white font-medium' : 'text-slate-400'}`}>
                       {item.text}
                     </p>
                  </SpotlightCard>
                ))}
      ...
      {/* --- Section 5: CTA --- */}
      <section id="diagnosis" className="h-[80vh] w-full flex flex-col justify-center items-center px-4 relative z-10">
        ...
        <div className="relative z-20 text-center max-w-2xl">
            ...
            <h2 className="text-3xl md:text-5xl font-light text-white mb-10 tracking-wide">
               Your first systemic win <br />begins with clarity.
            </h2>
